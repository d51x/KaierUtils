...
public class OBDCalculations {
    ...
    public static void sendOBD_AC_2110(int msg_id, Handler mHandler, ArrayList<Integer> buffer) {
        ...
        Log.w(TAG, "688 2110 Coolant Temperature: " + coolantTemp);
    }
    ...
    public static void sendOBD_AC_2113(int msg_id, Handler mHandler, ArrayList<Integer> buffer) {
        ...
        int rpm = buffer.get(3) * 256 + buffer.get(4);
...
//    }
    ...
    private static ClimateData.FanMode getAcFanMode(int x) {
        ClimateData.FanMode fanMode;
        int fan_mode = x & 0x0F; // bits 1-4, mask 0xF
    ...
    private static ClimateData.BlowMode getAcBlowMode(int x) {
    ...
    private static double getAcSetTemperature(int x) {
        return x / 2.0f - 50.0f;
    }

    public static void sendOBD_AC_2160(int msg_id, Handler mHandler, ArrayList<Integer> buffer) {
        if (buffer.isEmpty() || (buffer.size() < 4)) return;
        ClimateData climateData = new ClimateData();

        climateData.fan_mode = getAcFanMode(buffer.get(3));
        climateData.blow_mode = getAcBlowMode(buffer.get(3));
        climateData.temperature = getAcSetTemperature(buffer.get(2));
...
//    }
    ...
    private static ClimateData.BlowDirection getBlowDirection(int x) {
    ...
    private static ClimateData.FanSpeed getFanSpeed(int x) {
        ClimateData.FanSpeed fanSpeed = ClimateData.FanSpeed.unknown;

        switch (x & 0xF)   // bits 1-4, mask 0xF
        {
    ...
    private static ClimateData.State getAcState(int x) {
        return ( (x & 0x10) > 0 ) ? ClimateData.State.on : ClimateData.State.off;
    }

    private static ClimateData.State getRecirculationState(int x) {
        return ( ( x & 0x40 ) > 0 ) ? ClimateData.State.on : ClimateData.State.off;
    }

    private static ClimateData.State getDefoggerState(int x) {
        return ( ( x & 0x1 ) > 0 ) ? ClimateData.State.on : ClimateData.State.off;
    }

    public static void sendOBD_AC_2161(int msg_id, Handler mHandler, ArrayList<Integer> buffer) {
        if (buffer.isEmpty() || (buffer.size() < 4)) return;
        ClimateData climateData = new ClimateData();

        climateData.blow_direction = getBlowDirection(buffer.get(3));
        climateData.fan_speed = getFanSpeed(buffer.get(3));
        climateData.ac_state = getAcState(buffer.get(2));
        climateData.recirculation_state = getRecirculationState(buffer.get(2));
        climateData.defogger_state = getDefoggerState(buffer.get(2));
    ...
    public static void sendOBD_AC_2180(int msg_id, Handler mHandler, ArrayList<Integer> buffer) {
        if (buffer.isEmpty() || (buffer.size() < 5)) return;

        long condSysWorkTime =  (buffer.get(2) * 65536 + buffer.get(3) * 256 + buffer.get(4)) * 5;
...
//    }
    ...
    public static void sendOBD_Meter_21A3(int msg_id, Handler mHandler, ArrayList<Integer> buffer) {
        if (buffer.isEmpty() || (buffer.size() < 5)) return;
        if ( App.obd.obdData.fuel_tank > 0 )
            App.obd.canMmcData.can_mmc_fuel_remain = Math.round(( buffer.get(4) - 16) * (App.obd.obdData.fuel_tank / 100));
        else
